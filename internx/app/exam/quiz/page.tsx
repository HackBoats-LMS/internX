...
import { startExam, fetchQuestions, getExamConfig, submitExam as submitExamAction } from '@/app/exam/actions'
        ...
        const init = async () => {
            if (status === 'authenticated' && session?.user) {
                try {
                    ...
                    if (attempt.status === 'completed' || attempt.status === 'terminated') { router.push('/exam/result'); return }
    ...
    if (questions.length === 0) return (
        <div className="min-h-screen flex items-center justify-center bg-[#050505]">
            <div className="text-center p-10 bg-[#0d0d12] rounded-2xl shadow-lg max-w-sm">
                ...
                <button onClick={() => router.push('/exam')} className="mt-6 px-6 py-2.5 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors">Go Home</button>
            </div>
        </div>
    )
