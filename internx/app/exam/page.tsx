...
import Threads from '@/components/exam/ui/Threads'
...
export default function LoginPage() {
    ...
    setIsLoading(true)
      ; (async () => {
        try {
          ...
          if (profile?.role === 'admin') { router.push('/exam/admin'); return }
          const es = await checkExamStatus((session.user as any).id)
          router.push(es === 'completed' || es === 'terminated' ? '/exam/result' : '/exam/onboarding')
  ...
  const loginGoogle = () => {
    setIsLoading(true)
    signIn('google', { callbackUrl: '/exam/onboarding' })
  }
